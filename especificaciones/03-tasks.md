...
- [ ] Crear proyecto Supabase cloud real y hacer `supabase db push` cuando el equipo esté listo. Al crearlo: activar "Enable Data API" y "Enable automatic RLS", **desactivar** "Automatically expose new tables" (ver arriba).
- [x] Importar snapshot inicial de USDA FoodData Central a Supabase — 15/15 ingredientes (`supabase/scripts/import_usda.py`, corrido contra local con API key personal). Los ingredientes se fetchean por `fdcId` fijo, no por búsqueda de texto: la búsqueda por relevancia de USDA dio 2 matches incorrectos en el primer intento (confundió "sweet potato, raw" con "Sweet Potato puffs, frozen", y "oats" con "Oil, oat") — corregido pinneando el `fdcId` verificado de cada ingrediente en `seed_ingredients.py`.
