...
Con SIPSA e IPC descartados como fuente única/gratuita para el catálogo de ingredientes actual (proteínas/lácteos/granos/aceite), las opciones que quedan por evaluar son: (a) curar precios reales a mano desde listas de supermercado público (Éxito, Carulla, etc. publican precios en sus propios e-commerce, sin API oficial — requeriría scraping, zona gris de términos de servicio); (b) aceptar que el optimizador siga usando `purchase_unit_price` con precios placeholder indefinidamente y ser explícitos con el usuario final de que no son precios reales; (c) revisar si el mercado objetivo debería ampliarse a incluir USA (Kroger Developer API sí da precio real por SKU, pero solo esa cadena). Sin decidir todavía — mientras tanto, `ingredients.purchase_unit_price/label/size` se completan manualmente (son nullable por diseño).
