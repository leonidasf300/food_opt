...
The full stack is live end-to-end: `backend/` (Pyomo optimization, not deployed anywhere, run locally/from CI), `supabase/` (cloud project `food-opt`, ref `fufzybvwbdgejnozrgoa`, schema + USDA snapshot pushed), and `frontend/` deployed to Vercel at **https://food-opt.vercel.app**. The full user flow (magic-link login → move preference sliders → save) has been verified against production, not just locally — see `especificaciones/03-tasks.md` for the bugs that surfaced doing that and how they were fixed. Bugs found post-hoc are tracked as GitHub issues (see [issue #1](https://github.com/leonidasf300/food_opt/issues/1) for the first one).
