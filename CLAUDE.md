...
The `backend/` optimization module, the `supabase/` data layer schema, and the `frontend/` Next.js app have all been scaffolded and are under active development. The Supabase cloud project (`food-opt`, ref `fufzybvwbdgejnozrgoa`) exists, has the schema pushed, and has the 15-ingredient USDA snapshot imported — same as local (see `supabase/README.md`). Frontend isn't deployed anywhere yet.
